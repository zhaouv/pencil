...
以下结果均在 `6x6` 棋盘上, 使用 `node aivsai.js -n 100 -s` 的 200 局总样本得到:

+ `node aivsai.js -1 ro -2 ok -n 100 -s`
  `RolloutAI` 对 `OffensiveKeeperAI` 为 `161:39`, 胜率 `81%`, 总耗时约 `29.63s`
+ `node aivsai.js -1 ro -2 gr -n 100 -s`
  `RolloutAI` 对 `GreedyRandomAI` 为 `187:13`, 胜率 `94%`, 总耗时约 `30.02s`
