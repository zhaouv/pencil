...
- 当前主要问题：
  ...
  - 这条路径里的 `0/0/42` pure sacrifice 根，本轮已不再保留“同 simple 闭区域的重复 opening”：
    ...
    - 当前规则已改成：只对 `L2` 闭区域取中间边；其它 simple 闭区域直接保留首个代表边
  ...
  - 优先项：
    ...
    - 继续围绕 `ts_cases.js` 扩固定局面集，优先补：
      ...
      - 本轮又补了一层更窄的 exact sacrifice 排序修正：
        ...
        - 后续又把 simple 闭区域 canonical 收紧成“仅 `L2` 特判取中间，其他闭区域不再额外排序”
      ...
      - 本轮没有继续尝试“score-prefix 后继直接 canonical”，而是把范围收窄到 exact sacrifice root 的 simple region representative：
        ...
        - 当前规则已改成：只对 `L2` 闭区域取中间边，其它 simple 闭区域直接保留首个代表；simple region 若命中 `ok` 赢线首手，则对应代表边获得排序加成
